...
# @inline VectorizationBase.vload(ptr::VecStridedPointer) = @inbounds ptr.ptr[1 + ptr.offset].value
...
@inline VectorizationBase.stridedpointer(A::AbstractStrideArray{S,T,N,<:Tuple{1,Vararg}}) where {S,T,N} = VectorizationBase.PackedStridedPointer(pointer(A), VectorizationBase.staticmul(T,tailstrides(A)))
@generated function VectorizationBase.stridedpointer(A::AbstractStrideArray{S,T,N,X}) where {S,T,N,X}
    Xv = tointvec(X.parameters)
    ret = if last(Xv) == 1
        :(VectorizationBase.RowMajorStridedPointer{$T,$(N-1)}(pointer(A), revtailbytestrides(A)))
    # elseif first(Xv) == 1
        # :(VectorizationBase.PackedStridedPointer{$T,$N}(pointer(A), tailstrides(A)))
    else
        :(VectorizationBase.SparseStridedPointer{$T,$N}(pointer(A), tailbytestrides(A)))
    end
