...
struct StrideArray{S,T,N,X,SN,XN,V} <: AbstractMutableStrideArray{S,T,N,X,SN,XN,V}
    ptr::PtrArray{S,T,N,X,SN,XN,V}
    data::Vector{T}
end
@inline function StrideArray{S,T,N,X,SN,XN,V}(ptr, sz, sx, data) where {S,T,N,X,SN,XN,V}
    StrideArray{S,T,N,X,SN,XN,V}(PtrArray{S,T,N,X,SN,XN,V}(ptr, sz, sx), data)
end
...
@inline function FixedSizeArray{S,T,N,X,L}(::UndefInitializer) where {S,T,N,X,L}
    ...
    FixedSizeArray{S,T,N,X,L,0,0,false}(PtrArray{S,T,N,X,0,0,false}(ptr, tuple(), tuple()), r)
end
@inline function FixedSizeArray{S,T,N,X,SN,XN,V}(ptr::Ptr{T}, sz::NTuple{SN,Int}, st::NTuple{XN,Int}, data::Base.RefValue{NTuple{L,T}}) where {S,T,N,X,L,SN,XN,V}
    FixedSizeArray{S,T,N,X,L,SN,XN,V}(PtrArray(ptr, sz, st), data)
end
