...
@inline _call_loopmul!(C, A, B, α, β, M, K, N, ::Val{true}) = packaloopmul!(C, A, B, α, β, M, K, N)
...
function (::SyncMulFunc{TC,TA,TB,Α,Β,Md,Kd,Nd,AP,BCP,ID,TT,W₁,W₂,R₁,R₂})(p::Ptr{UInt}) where {P,TC,TA,TB,Α,Β,Md,Kd,Nd,AP,BCP,ID,TT,W₁,W₂,R₁,R₂}
    offset, C = _atomic_load(p, 1, TC)
    offset, A = _atomic_load(p, offset, TA)
    offset, B = _atomic_load(p, offset, TB)
    offset, α = _atomic_load(p, offset, Α)
