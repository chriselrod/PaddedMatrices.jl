...
@generated function Random.rand!(rng, A::AbstractMutableFixedSizePaddedArray{S,T,N,R,L}) where {S,T<:Union{Float32,Float64},N,R,L}
    size_T = sizeof(T)
...
@generated function Random.rand(rng, ::Type{ <: ConstantFixedSizePaddedArray{S,T}}) where {S,T<:Union{Float32,Float64}}
...
@generated function Random.randn!(rng, A::AbstractMutableFixedSizePaddedArray{S,T,N,R,L}) where {S,T<:Union{Float32,Float64},N,R,L}
    size_T = sizeof(T)
    W = VectorizationBase.pick_vector_width(L, T)
    nrep, r = divrem(L, 4W)
...
@generated function Random.randn(rng, ::Type{<:ConstantFixedSizePaddedArray{S,T}}) where {S,T<:Union{Float32,Float64}}
    N = length(S.parameters)
