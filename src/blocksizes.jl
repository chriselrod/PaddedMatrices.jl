...
const W₁Default = 0.0125
const W₂Default = 0.025
const R₁Default = 0.5
const R₂Default = 0.8
...
"""
@inline function solve_block_sizes(::Type{T}, M, K, N, _α, _β, R₂, R₃) where {T}
    W = VectorizationBase.pick_vector_width_val(T)
    α = _α * W
    β = _β * W
    L₁ₑ =  first_effective_cache(T) * R₂
    ...
    # Kc_init = Base.FastMath.min_fast(√(L₁ₑ/α), L₂ₑ/Nblock_Nrem)
    # Kiter = cld(K, Kc_init)
    Kc_init⁻¹ = Base.FastMath.max_fast(√(α/L₁ₑ), Nblock_Nrem*inv(L₂ₑ))
    Kiter = cldapproxi(K, Kc_init⁻¹) # approximate `ceil`
    Kblock, Krem = divrem_fast(K, Kiter)
    Kblock_Krem = Kblock + One()#(Krem > 0)

    Miter_init = cldapproxi(M * inv(L₁ₑ), Kblock_Krem)
    Mbsize, Mrem, Mremfinal, Miter = split_m(M, Miter_init, W)

    promote(Mbsize, Mrem, Mremfinal, Miter), promote(Kblock, Kblock_Krem, Krem, Kiter), promote(Nblock, Nblock_Nrem, Nrem, Niter)
end
